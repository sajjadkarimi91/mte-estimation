...
function [PTE ,I , PMI , NCS , sig_te]...
    = te_embed_lsim( pi_0_lsim , coupling_tetha_IM , transition_matrices_IM ,  gmm_para_lsim  ,  Y_in, extra  )
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
index_channels = extra.index_channels;
index_lags = extra.index_lags;
num_channels = length(unique(index_channels));
ot_channels = find(index_lags == 0);

if length(ot_channels) ~= num_channels
    error('Please check lag and channel structure')
end
...
H_ot_Opast = zeros(num_channels,1);
H_ot_Opast(index_channels(ot_channels)) = - mean(log(P_ot_c_cond_past(ot_channels,min_order:end)),2,'omitnan');

for ch = 1:num_channels
    c = ot_channels(ch);
    P_ot_c_cond_past_temp(c,:) = sum(alpha_steady{c,1}.*P_observ_cond_to_state_out{c,1});
    H_ot(index_channels(c),1) =  -mean(log(P_ot_c_cond_past_temp(c,min_order:end)),2,'omitnan');
end
...
for ch = 1:num_channels
    ...
    Y_in_temp = Y_in;
    for z=1:C
        if index_channels(z)== index_channels(ot_channels(ch))
            Y_in_temp{z,1} = nan*Y_in_temp{z,1};
        end
    end

    P_ot_c_cond_past_temp = mc_lsim( pi_steady_lsim , coupling_tetha_IM , transition_matrices_IM ,  gmm_para_lsim  ,  Y_in_temp  );
    ...
    H_ot_Opast_partial(index_channels(c),index_channels(ot_channels)) =  (-mean(log(P_ot_c_cond_past_temp(ot_channels,min_order:end)),2,'omitnan'))';

end

PTE = H_ot_Opast_partial - repmat(H_ot_Opast',num_channels,1);
PTE = PTE - diag(diag(PTE));
I = H_ot - H_ot_Opast;
PMI = PTE./repmat(I',num_channels,1);
