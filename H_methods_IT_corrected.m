function output = H_methods_IT_corrected ( data, config, waitbar )
...
%     Nlags = channels-1;
% else
Nlags = config.Nlags; % 1, to save time but...
% end
...
for window = 1: windows
    for ch1 = 1: channels-1
        ...
        %%%%
        celldata_ch1 = delay_embed( celldata_ch1, config.EmbDim, config.TimeDelay);
        %%%%
        ...
        for ch2 = ch1 + 1: channels
            ...
            %%%%
            celldata_ch2 = delay_embed( celldata_ch2, config.EmbDim, config.TimeDelay);
            %%%%
            ...
            if PMI_on
                ...
                for n3 = 1: size( Groups, 1 )
                    ...
                    %%%%
                    celldata_ch3 = delay_embed( celldata_ch3, config.EmbDim, config.TimeDelay);
                    %%%%
